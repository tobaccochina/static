...
Cms.f7.node = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree.do";
...
Cms.f7.nodeNodePerm = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree_node_perms.do";
	var settings = {title:"Node Select",width:350,height:450};
	options.settings = $.extend(settings, options.settings);
	var names = ["Number","Name"];
	var f7 = Cms.F7Single(url,name,names,focus,options);
...
Cms.f7.nodeInfoPerms = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree_info_perms.do";
	var settings = {title:"Node Select",width:350,height:450};
	options.settings = $.extend(settings, options.settings);
	var names = ["Number","Name"];
	var f7 = Cms.F7Single(url,name,names,focus,options);
...
Cms.f7.nodeMulti = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree_multi.do";
...
Cms.f7.nodeMultiInfoPerms = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree_multi_info_perms.do";
	var settings = {title:"Node Select",width:550,height:450};
	options.settings = $.extend(settings, options.settings);
	var f7 = Cms.F7Multi(url, name, options);
}
...
Cms.f7.perm = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/role/choose_perm_tree.do";
...
Cms.f7.nodePerms = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/node/choose_node_tree_perms.do";
	var settings = {title:"Node Select",width:350,height:450};
	options.settings = $.extend(settings, options.settings);
	var f7 = Cms.F7Multi(url, name, options);
};
...
Cms.f7.org = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/org/choose_org_tree.do";
...
Cms.f7.orgMulti = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/org/choose_org_tree_multi.do";
...
Cms.f7.template = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/web_file/choose_template_tree.do";
...
Cms.f7.style = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/web_file/choose_style_tree.do";
	var settings = {title:"WebFile Select",width:350,height:450};
	options.settings = $.extend(settings, options.settings);
	var names = [];
	var f7 = Cms.F7Single(url,name,names,name,options);
...
Cms.f7.specialMulti = function(name, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/special/choose_special_multi.do";
...
Cms.f7.site = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/site/choose_site_tree.do";
...
Cms.f7.uploads = function(name, focus, options) {
	options = options || {};
	var url = CTX + ADMIN_LOP + "/core/web_file/choose_uploads.do";
...
$(function(){
	//关联下拉
	if($("input[name='customs_BigBusinessName']").length>0&&$("input[name='customs_selectbrand']").length>0)
	{
		var bv=$("input[name='customs_BigBusinessName']").val(),sv=$("input[name='customs_selectbrand']").val();
	    change_BigBusinessName();
	    change_SmallCustomsName(bv,'customs_BigBusinessName',false);
	
	}
	//
	if($("input[name='customs_edit']").length>0)
	{
		var t=$("input[name='customs_edit']");
		t.hide();
		t.parent().append("<span>"+t.val()+"</span>");
	}
	
})
function change_BigBusinessName(){
		var url = CTX + ADMIN_LOP + "/plug/enterprise/choose_enterprise.do?dt="+(new Date()).getTime()+"";
		$.getJSON(url, function(json){
			json=eval(json);
            var t=$("input[name='customs_BigBusinessName']");
            t.hide();
            var str="";
            for(var p in json){
            	if(t.val()!=0&&t.val()==json[p].id)
                str+="<option value=\""+json[p].id+"\" selected=\"selected\">"+json[p].name+"</option>";
                else
                str+="<option value=\""+json[p].id+"\">"+json[p].name+"</option>";
            }
            t.parent().append("<select class=\"form-control customs_BigBusinessName\" onchange=\"change_SmallCustomsName(this.value,'customs_BigBusinessName',true)\"><option value=\"0\">无</option>"+str+"</select>");
      });
}
function change_SmallCustomsName(v,str,s)
{

		var t=$("input[name='customs_selectbrand']");
		if(s){$("input[name='"+str+"']").val(v);console.log($("input[name='"+str+"']").val())}else{t.parent().append("<select class=\"form-control customs_selectbrand\" onchange=\"change_selectbrand(this.value,'customs_selectbrand')\"></select>");}
		var url = CTX + ADMIN_LOP + "/plug/brand/choose_brand.do?dt="+(new Date()).getTime()+"&tid="+v;
		$.getJSON(url, function(json){
			json=eval(json);
            t.hide();
            var str="";
            ...
            $(".customs_selectbrand").html("<option value=\"0\">无</option>"+str+"");
      });
	
}
function change_selectbrand(v,str)
{
	$("input[name='"+str+"']").val(v);
	//console.log($("input[name='"+str+"']").val())
}
